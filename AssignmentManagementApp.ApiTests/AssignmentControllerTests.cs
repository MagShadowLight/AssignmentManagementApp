...
namespace AssignmentManagementApp.ApiTests
{
    public class AssignmentControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        [Fact]
        public async Task When_API_Get_Assignments_Then_It_Should_Return_Notes()
        {
            // Arrange
            var json = new StringContent(
                JsonSerializer.Serialize(new { title = "Task", description = "do something", priority = Priority.Low, notes = "Test Note"}),
                Encoding.UTF8, "application/json");
            await _httpClient.PostAsync("/api/Assignment", json);
            var getResponse = await _httpClient.GetAsync("/api/Assignment");
            // Act
            ...
            // Assert
            getResponse.EnsureSuccessStatusCode();
            Assert.Contains(assignment, assignments => assignments.Notes == "Test Note");
        }
    }
}